...
int client(char *argv[])
{
    ...
    serverAddress6.sin6_port = htons(atoi(argv[3]));

    // Convert IPv4 and IPv6 serverAddress6es from text to binary form
    if (inet_pton(AF_INET6, argv[2], &serverAddress6.sin6_addr) <= 0)
    {
        printf("\nInvalid serverAddress6/ serverAddress6 not supported \n");
        return -1;
    }
    ...
    {
        if (sendto(sock, buffer, read_bytes, 0, (struct sockaddr *)&serverAddress6, sizeof(serverAddress6)) == -1)
        {
...
int server(char *argv[])
{
    ...
    struct sockaddr_in6 serverAddress6;
    // int addrlen = sizeof(serverAddress6);
    memset(&serverAddress6, 0, sizeof(serverAddress6));
    serverAddress6.sin6_family = AF_INET6;
    serverAddress6.sin6_addr = in6addr_any;
    serverAddress6.sin6_port = htons(atoi(argv[2]));

    // Bind the socket to the specified serverAddress6
    if (bind(server_fd, (struct sockaddr *)&serverAddress6, sizeof(serverAddress6)) < 0)
    {
        perror("bind failed");
        exit(EXIT_FAILURE);
    }
    ...
    printf("IPV6 UDP milliseconds ??? : %ld\n", elapsed_time);
